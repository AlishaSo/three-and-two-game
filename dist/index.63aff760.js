...
const drawCard = async (count)=>{
    try {
        let info = await (0, _axiosDefault.default).get(`${BASE_URL}${deckId}/draw/?count=${count}`);
...
const addCardToApiPile = async ()=>{
    try {
        await (0, _axiosDefault.default).get(`${BASE_URL}/${deckId}/pile/${discardListName}/add/?cards=${discardCard.code}`);
    } catch (e) {
        console.error(e);
    }
};
...
const addEventListenerToUsersCards = (e)=>{
    ...
    for(let n = 0; n < player1Hand.length; n++)if (player1Hand[n].code == currentCardIndex) {
        currentCardIndex = n;
        break;
    }
    if (pickedUpCard != null) {
...
function shouldComputerTakeDiscard(countObj) {
    ...
    if (computersHand.some((c)=>c.value == temp.value) && countObj[temp.value].count < 3) {
        cardNumExists = true;
        for(const key in countObj)if (key != temp.value && countObj[key].count == 1) {
...
function computerPickUp(countObj) {
    ...
    if (computersHand.some((c)=>c.value == tempPick.value) && countObj[tempPick.value].count < 3) {
        cardNumExists = true;
        for(const key in countObj)if (key != tempPick.value && countObj[key].count == 1) {
            foundIndex = countObj[key].index;
            break;
        }
        // addCardToApiPile()
...
const playGame = async ()=>{
    ...
    while(keepPlaying)if (usersTurn) {
        ...
        computersHand.forEach((card, index)=>{
            countObj[card.value] ? countObj[card.value].count += 1 : countObj[card.value] = {
                count: 1,
                index
            };
        });
...
},{"./../utils":"5By4s"}]},["1JEHZ","adjPd"], "adjPd", "parcelRequired35d")
